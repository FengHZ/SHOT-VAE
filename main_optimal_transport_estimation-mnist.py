...
from lib.dataloader import get_ssl_sampler, svhn_dataset, mnist_dataset
import os
from os import path
import time
import shutil
import ast
import numpy as np
from lib.utils.mixup import mixup_data
...
parser.add_argument('--dataset', default="MNIST", type=str, help="The dataset name")
...
parser.add_argument('-sam', "--supervised-alpha-max", default=1, type=float,
                    help="the max weight(alpha) for the balance between supervised and unsupervised loss")
parser.add_argument('-uam', "--unsupervised-alpha-max", default=1, type=float,
                    help="the max weight(alpha) for the balance between supervised and unsupervised loss")
parser.add_argument('-uame', '--unsupervised-alpha-modify-epoch', default=80, type=int,
                    help="weight(alpha) will get alpha-max at amf * epochs")
parser.add_argument('--ar', '--annotated-ratio', default=0.1, type=float, help="The annotated ratio for SSL")
...
parser.add_argument('--mixup', default=True, type=bool, help="use mixup method")
...
parser.add_argument('-ad', "--adjust-lr", default=[100, 150, 180], type=arg_as_list,
...
def main(args=args):
    ...
    if args.dataset == "MNIST":
        ...
        sampler_valid, sampler_train_l, sampler_train_u = get_ssl_sampler(
            torch.tensor(train_dataset.targets.clone().detach(), dtype=torch.int32), 100, 10, 10)
        ...
        num_classes = 10
        model = mlp.MLP(data_parallel=args.dp, drop_rate=args.dr)
        model = model.cuda()
        ...
        sampler_valid, sampler_train_l, sampler_train_u = get_ssl_sampler(
            torch.tensor(train_dataset.labels.clone().detach(), dtype=torch.int32), 100, 100, 10)
        test_dloader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)
        valid_dloader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True,
                                   sampler=sampler_valid)
        train_dloader_l = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.workers,
        ...
        train_dloader_u = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.workers,
                                     ...
                                     sampler=sampler_train_u)
        num_classes = 10
        model = mlp.MLP(num_input_channels=1024, num_classes=num_classes, data_parallel=args.dp, drop_rate=args.dr)
        model = model.cuda()
    else:
        raise NotImplementedError("Dataset {} Not Implemented".format(args.dataset))

    print(
        "Begin the {} time's semi-supervised training, Dataset:{}".format(args.train_time, args.dataset,
                                                                          args.mixup))
    ...
    writer_log_dir = "{}/{}-OPE/runs/train_time:{}".format(args.base_path, args.dataset, args.train_time)
    if args.resume:
        ...
        if os.path.exists(writer_log_dir):
            flag = input("{}-OPE train_time:{} will be removed, input yes to continue:".format(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_dloader_l, train_dloader_u, model=model, criterion_l=criterion_l, criterion_u=criterion_u,
              optimizer=optimizer, epoch=epoch, writer=writer, alpha=alpha)
        # if epoch >= 400:
...
def train(train_dloader_l, train_dloader_u, model, criterion_l, criterion_u, optimizer, epoch, writer, alpha):
    ...
    for (image_l, label_l), (image_u, _) in zip(cycle(train_dloader_l), train_dloader_u):
        ...
        if args.mixup:
            ...
            loss_supervised = lam * criterion_l(cls_result_l, label_a) + (1 - lam) * criterion_l(cls_result_l, label_b)
            ...
            # label_u_approx_mixup = lam * label_a_approx + (1 - lam) * label_b_approx
            # loss_unsupervised = torch.nn.functional.mse_loss(cls_result_u, label_u_approx_mixup)
            loss = args.supervised_alpha_max * loss_supervised + alpha * loss_unsupervised
        else:
...
def save_checkpoint(state, filename='checkpoint.pth.tar'):
    ...
    :return:
    """
    filefolder = "{}/{}-OPE/parameter/train_time:{}".format(args.base_path, args.dataset, args.train_time)
...
def alpha_schedule(epoch):
    alpha = args.unsupervised_alpha_max * math.exp(-5 * (1 - min(1, epoch / args.unsupervised_alpha_modify_epoch)) ** 2)
    return alpha
